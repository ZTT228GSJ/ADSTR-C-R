...
def scaled_Laplacian(W):#计算对称归一化的拉普拉斯矩阵（缩放操作）
    '''
    compute \tilde{L}
...
def cheb_polynomial(L_tilde, K):#计算切比雪夫多项式（用于图卷积核）
    '''
...
# def compute_val_loss_mstgcn(net, val_loader, criterion, sw, epoch, limit=None):
    ...
    with torch.no_grad():
        ...
        for batch_index, batch_data in enumerate(val_loader):
            ...
            # 将msked_flag 和 missing_value 注释掉；似乎导致了损失值的计算方式变化
            # if masked_flag:
...
def predict_and_save_results_mstgcn(net, data_loader, data_target_tensor, global_step, metric_method,_mean, _std, params_path, type):
    ...
    with torch.no_grad():
        ...
        if metric_method == 'mask':
            ...
            mape = masked_mape_np(data_target_tensor.reshape(-1, 1), prediction.reshape(-1, 1), 0.0)
        else :

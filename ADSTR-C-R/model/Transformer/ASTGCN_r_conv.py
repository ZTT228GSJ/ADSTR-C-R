...
class ASTGCN_block(nn.Module):
    ...
    def __init__(self, DEVICE, in_channels, K, nb_chev_filter, nb_time_filter, time_strides, cheb_polynomials, num_of_vertices, num_of_timesteps):
        ...
        # # 添加ConvLSTM模块
        self.convlstm = ConvLstm(3,5,3, 1, False, True, False)
        self.residual_conv = nn.Conv2d(in_channels, nb_time_filter, kernel_size=(1, 1), stride=(1, time_strides))

        # 添加Transfomer模块
        # self.transformer = greedy_decode()


        # LSTM
        # self.residual_lstm = nn.LSTM(input_size=nb_time_filter, hidden_size=3,
        #                     num_layers=3, batch_first=True)

        self.W1 = nn.Parameter(torch.FloatTensor(307,12).to(DEVICE))
        self.W2 = nn.Parameter(torch.FloatTensor(307,12).to(DEVICE))
    ...
    def forward(self, x):
        ...
        # ConvLSTM操作
        # 在此处为了不让网络遗忘初始数据信息又不至于受初始信息影响过大，对X取其值的的百分比.



        time_conv_output = (time_conv_output+x_residual*self.W1).unsqueeze(2)





        output =  self.convlstm(time_conv_output.permute(3,0,2,1,4))
        output = (output[0][0])[:,:,-1,:,:].permute(0,2,1,3)

          # (b,N,F,T)->(b,F,N,T) 用(1,1)的卷积核去做->(b,F,N,T)

        x_residual = self.ln(F.relu(x_residual*self.W2 + output).permute(0, 3, 2, 1)).permute(0, 2, 3, 1)
...
class ASTGCN_submodule(nn.Module):
    ...
    def forward(self, x):
        ...
        for block in self.BlockList:
            x = block(x)
        # x = x.unsqueeze(2)
        # output =  self.convlstm(x.permute(3,0,2,1,4))
